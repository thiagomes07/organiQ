...
export function useArticles(filters: ArticleFilters = {}) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
      ...
      updateUser({
        articlesUsed: (articlesQuery.data?.articles.length || 0) - 1,
      });
  ...
  const getArticlesByStatus = (status: ArticleStatus) => {
    return (
      articlesQuery.data?.articles.filter(
  ...
  const hasActiveArticles = () => {
    return (
      articlesQuery.data?.articles.some(
        (article) =>
          article.status === "generating" || article.status === "publishing"
      ) || false
    );
  ...
  return {
    ...
    activeCount:
      getArticlesByStatus("generating").length +
      getArticlesByStatus("publishing").length,
  };
}
